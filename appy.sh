...
install_immich() {
  ...
  command -v docker-compose &>/dev/null || {
    warn "docker-compose not found — installing Docker Compose first..."
    do_install compose
  }

  local immich_dir="/var/lib/immich"
  local upload_dir="${immich_dir}/upload"
  ...
  # Write .env file (never echo secrets to terminal)
  cat > "$env_file" <<EOF
# Immich environment — generated by appy v${APPY_VERSION} on $(date)
...
DCEOF
  ...
  # Launch Immich stack
  spin_start "Pulling Immich images (first pull can take a few minutes)..."
  docker-compose -f "$compose_file" --env-file "$env_file" pull 2>&1 | tee -a "$APPY_LOG" > /dev/null || true
  spin_stop

  spin_start "Starting Immich services..."
  docker-compose -f "$compose_file" --env-file "$env_file" up -d 2>&1 | tee -a "$APPY_LOG" > /dev/null || true
  spin_stop

  # Save credentials
  {
    printf "=== Immich ===\nGenerated: %s\nURL: http://localhost:2283\nUpload dir: %s\nDB password: %s\nCompose file: %s\nEnv file: %s\n\n" \
      "$(date)" "$upload_dir" "$db_pass" "$compose_file" "$env_file"
  } >> "$CRED_FILE"
  chmod 600 "$CRED_FILE"

  _print_info_card "📷  Immich Photo Manager Installed" \
    "Web UI:        http://localhost:2283  (may take 1-2 min to start)" \
    "Upload dir:    ${upload_dir}" \
    "Compose file:  ${compose_file}" \
    "  → Create your admin account on first visit" \
    "  → Install mobile app: search 'Immich' on App Store / Google Play" \
    "  → AI features: face recognition, object search, smart albums" \
    "  → Credentials saved to:  ~/appy-credentials.txt" \
    "  → ⚠  Put behind HTTPS reverse proxy before exposing externally" \
    "  → Manage:  docker-compose -f ${compose_file} ps"
}
...
run_auto_update() {
  ...
  if command -v docker &>/dev/null && systemctl is-active --quiet docker 2>/dev/null; then
    ...
    if [[ -f /var/lib/immich/docker-compose.yml ]]; then
      ...
      docker-compose -f /var/lib/immich/docker-compose.yml \
        --env-file /var/lib/immich/.env pull 2>/dev/null | grep -E "Pulling|Status:" || true
      docker-compose -f /var/lib/immich/docker-compose.yml \
        --env-file /var/lib/immich/.env up -d 2>/dev/null || true
    fi
...
run_rollback_menu() {
  ...
  case "$type" in
    ...
    docker)
      ...
      # Special case: Immich uses compose
      if [[ "$chosen_key" == "immich" && -f /var/lib/immich/docker-compose.yml ]]; then
        docker-compose -f /var/lib/immich/docker-compose.yml down 2>/dev/null || true
      fi
      ;;
  esac
